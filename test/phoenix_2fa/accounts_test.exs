defmodule Phoenix2FA.AccountsTest do
  use Phoenix2FA.DataCase

  alias Phoenix2FA.Accounts

  import Phoenix2FA.AccountsFixtures
  alias Phoenix2FA.Accounts.{User, UserToken}
  ...
  describe "user_keys" do
    ...
    @invalid_attrs %{cred_id: nil, kind: nil, label: nil, last_used: nil, mfa_key: nil}

    test "list_user_keys/0 returns all user_keys" do
      user_key = user_key_fixture()
      assert Accounts.list_user_keys() == [user_key]
    end

    test "get_user_key!/1 returns the user_key with given id" do
      user_key = user_key_fixture()
      assert Accounts.get_user_key!(user_key.id) == user_key
    end
    ...
    test "create_user_key/1 with valid data creates a user_key" do
      ...
      assert {:ok, %UserKey{} = user_key} = Accounts.create_user_key(valid_attrs)
    ...
    test "create_user_key/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Accounts.create_user_key(@invalid_attrs)
    end
    ...
    test "update_user_key/2 with valid data updates the user_key" do
      ...
      assert {:ok, %UserKey{} = user_key} = Accounts.update_user_key(user_key, update_attrs)
    ...
    test "update_user_key/2 with invalid data returns error changeset" do
      user_key = user_key_fixture()
      assert {:error, %Ecto.Changeset{}} = Accounts.update_user_key(user_key, @invalid_attrs)
      assert user_key == Accounts.get_user_key!(user_key.id)
    end

    test "delete_user_key/1 deletes the user_key" do
      user_key = user_key_fixture()
      assert {:ok, %UserKey{}} = Accounts.delete_user_key(user_key)
      assert_raise Ecto.NoResultsError, fn -> Accounts.get_user_key!(user_key.id) end
    end

    test "change_user_key/1 returns a user_key changeset" do
      user_key = user_key_fixture()
      assert %Ecto.Changeset{} = Accounts.change_user_key(user_key)
    end
  end
end
